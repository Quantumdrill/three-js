...
function gsapStuff() {
	...
	LEFT_BUTTON.addEventListener('click', () => {
		gsap.to(meshes.flower.position, {
			x: meshes.flower.position.x - 0.5,
			duration: 1,
			ease: 'power1',
		})
	})
	...
	ANIMATE_BUTTON.addEventListener('click', () => {
		gsap.to(meshes.flower.position, {
			x: meshes.flower.position.y + 0.5,
			duration: 1,
			ease: 'power1',
		})
	...
	SCALE_BUTTON.addEventListener('click', () => {
		gsap.to(meshes.flower.scale, {
			x: meshes.flower.scale.x - 0.2,
			y: meshes.flower.scale.y - 0.2,
			z: meshes.flower.scale.z - 0.2,
			duration: 0.5,
			ease: 'power3',
		})
	})
